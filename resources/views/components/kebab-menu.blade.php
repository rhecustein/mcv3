...
<div x-data="{ open: false }" class="relative inline-block text-left">
    ...
    <div x-show="open" @click.away="open = false" 
        ...
        <div class="py-1" role="menu" aria-orientation="vertical">
            <a href="{{ route('superadmin.outlets.edit', $outlet) }}" class="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100" role="menuitem">
            ...
            <form action="{{ route('superadmin.outlets.toggle', $outlet) }}" method="POST" class="w-full">
                @csrf @method('PATCH')
            ...
            <form action="{{ route('superadmin.outlets.reset-password', $outlet) }}" method="POST" onsubmit="return confirm('Reset password akun user outlet ini ke default?')" class="w-full">
                @csrf
            ...
            <form action="{{ route('superadmin.outlets.destroy', $outlet) }}" method="POST" onsubmit="return confirm('PERINGATAN: Yakin ingin menghapus permanen outlet ini?')" class="w-full">
                @csrf @method('DELETE')
