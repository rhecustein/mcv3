...
<div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-slate-200">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">Riwayat Transaksi Paket</h1>
            <p class="mt-2 text-slate-500">Pantau semua transaksi pembelian paket langganan oleh perusahaan klien.</p>
        </div>
    </div>
    ...
    <div class="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-6">
        <h2 class="text-lg font-semibold mb-4 text-slate-800">📊 Grafik Pendapatan Tahunan</h2>
        <div class="h-80">
    ...
    <div class="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-4">
        <form method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <a href="{{ route('superadmin.package-transactions.index') }}" class="w-full md:w-auto text-center px-4 py-2 bg-slate-200 text-slate-700 rounded-md text-sm font-medium hover:bg-slate-300 transition">Reset</a>
        </form>
    </div>
                        ...
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <img class="h-10 w-10 rounded-full bg-slate-200 object-cover" src="{{ $trx->company->logo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($trx->company->name ?? 'P').'&background=e2e8f0&color=475569' }}" alt="Logo">
                                    <div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-slate-500 text-xs">
                                    ID: <span class="font-mono">{{ $trx->id }}</span> | Tgl: {{ $trx->created_at->format('d M Y') }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <a href="#" class="px-3 py-1.5 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-md font-medium text-xs">
                                    Lihat Detail
                                </a>
                            </td>
                        </tr>
