...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800">Edit Outlet: {{ $outlet->name }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div class="space-y-6 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $outlet->email) }}"
            ...
            <div class="space-y-6 bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="4"
                              class="block w-full rounded-md border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('address') border-red-500 @enderror">{{ old('address', $outlet->address) }}</textarea>
                    @error('address')
                        <p class="text-sm text-red-600 mt-2">{{ $message }}</p>
                    @enderror
                </div>
         ...
         <div class="bg-red-50 border border-red-200 rounded-xl p-6">
            <div class="flex flex-col md:flex-row gap-4 md:items-center">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-red-700 mt-1">Tindakan ini tidak dapat diurungkan. Menghapus outlet akan menghapus data mereka secara permanen.</p>
                </div>
                <div class="flex-shrink-0 mt-4 md:mt-0">
                    <form action="{{ route('outlets.destroy', $outlet) }}" method="POST" onsubmit="return confirm('PERINGATAN! Anda akan menghapus outlet ini secara permanen. Lanjutkan?');">
                        @csrf
                        @method('DELETE')
