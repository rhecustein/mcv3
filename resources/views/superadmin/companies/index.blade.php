...
<div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-slate-200">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">Manajemen Perusahaan</h1>
            <p class="mt-2 text-slate-500">Kelola perusahaan klien, paket langganan, dan status kerja sama.</p>
        </div>
        <a href="{{ route('superadmin.companies.create') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition-all duration-200">
    ...
    <div class="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-4">
        <form method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <a href="{{ route('superadmin.companies.index') }}" class="w-full md:w-auto text-center px-4 py-2 bg-slate-200 text-slate-700 rounded-md text-sm font-medium hover:bg-slate-300 transition">Reset</a>
        </form>
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
            <div>
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h3 class="font-bold text-slate-800 text-lg">{{ $company->name }}</h3>
                        <p class="text-sm text-slate-500">{{ $company->industry_type ?? 'Industri tidak spesifik' }}</p>
                    </div>
                        ...
                        <div x-data="{ open: false }" class="relative inline-block text-left">
                            ...
                            <div x-show="open" @click.away="open = false" x-cloak x-transition class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                                <div class="py-1" role="menu">
                                    <a href="{{ route('superadmin.companies.edit', $company) }}" class="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100" role="menuitem">
                                    ...
                                    <form action="{{ route('superadmin.companies.destroy', $company) }}" method="POST" onsubmit="return confirm('PERINGATAN: Yakin ingin menghapus perusahaan ini?')" class="w-full">
                                        @csrf @method('DELETE')
                ...
                <div class="text-sm text-slate-600 space-y-3">
                    <p><strong>Kontak:</strong> {{ $company->email ?? $company->phone ?? 'N/A' }}</p>
                    <p><strong>Paket:</strong> <span class="font-medium text-indigo-600">{{ $company->package->name ?? 'Belum berlangganan' }}</span></p>
                    ...
                    <div>
                        <div class="flex justify-between text-xs mb-1">
                            <span class="font-medium text-slate-700">Penggunaan Kuota</span>
                            <span>{{ $letterUsage }} / {{ $maxLetters > 0 ? $maxLetters : '∞' }}</span>
                        </div>
                        <div class="w-full bg-slate-200 rounded-full h-2">
                            <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $letterPercentage }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
             <div class="mt-5 pt-4 border-t border-slate-200 flex justify-between items-center">
                <div class="text-xs text-slate-500">
                    @if ($company->package_end_date)
                        Aktif hingga: <span class="font-medium text-slate-700">{{ $company->package_end_date->format('d M Y') }}</span>
                    @else
                        Tidak berlangganan
                    @endif
                </div>
        ...
        <div class="md:col-span-2 xl:col-span-3 text-center text-slate-500 py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
            <svg class="w-16 h-16 mx-auto text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h6M9 11.25h6M9 15.75h6M9 20.25h6" /></svg>
            <p class="mt-4 font-semibold">Belum Ada Perusahaan Klien Terdaftar</p>
            <p class="text-sm">Gunakan tombol "Tambah Perusahaan" untuk memulai.</p>
        </div>
        @endforelse
    </div>
