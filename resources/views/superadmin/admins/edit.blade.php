    ...
    <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
        <form action="{{ route('admins.update', $admin) }}" method="POST" id="editAdminForm">
            ...
            <div class="mb-8 pb-6 border-b border-slate-200">
                <h1 class="text-2xl font-bold text-slate-900">Edit Data: {{ $admin->user->name }}</h1>
                <p class="mt-1 text-sm text-slate-500">Perbarui detail akun dan informasi administratif untuk pengguna ini.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        @error('region_name')<p class="text-sm text-red-600 mt-2">{{ $message }}</p>@enderror
                    </div>
                    <div>
                ...
                <div class="md:col-span-2 mt-4 pt-6 border-t border-slate-200" x-data="{ showPasswordFields: false }">
                    <button type="button" @click="showPasswordFields = !showPasswordFields" class="text-sm font-semibold text-slate-600 hover:text-slate-900 flex items-center gap-2">
                        <span>Ubah Password</span>
                        <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-180': showPasswordFields }" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                    </button>
                    <div x-show="showPasswordFields" x-transition class="mt-4 space-y-4 max-w-sm">
                        <div>
                            <label for="password" class="block text-sm font-medium leading-6 text-slate-700 mb-1.5">Password Baru</label>
                            ...
                            <p class="text-xs text-slate-500 mt-2">Kosongkan jika tidak ingin mengubah password.</p>
        ...
        <div class="bg-red-50 border border-red-300 rounded-2xl p-6">
            <div class="flex flex-col md:flex-row gap-4 md:items-center">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-red-700 mt-1">Tindakan menghapus admin akan menghapus data mereka secara permanen dan tidak dapat diurungkan.</p>
                </div>
                <div class="flex-shrink-0 mt-4 md:mt-0">
                    <form action="{{ route('admins.destroy', $admin) }}" method="POST" onsubmit="return confirm('PERINGATAN! Anda akan menghapus admin ini secara permanen. Lanjutkan?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition">
