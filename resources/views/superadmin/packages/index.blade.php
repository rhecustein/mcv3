...
<div class="bg-white p-6 sm:p-8 rounded-2xl shadow-lg">
    ...
    <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        @php
            ...
            $mostPopularPackage = $packages->sortByDesc('companies_count')->first();
            $activeSubscriptions = $totalActiveCompanies ?? 0; // Ganti dengan data dari controller
        @endphp
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 flex items-center gap-5">
            <div class="w-12 h-12 flex-shrink-0 flex items-center justify-center bg-sky-100 rounded-full">
        ...
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 flex items-center gap-5">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-slate-800 mt-1 truncate">{{ $mostPopularPackage->name ?? 'N/A' }}</p>
            </div>
        </div>
        <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 flex items-center gap-5">
            <div class="w-12 h-12 flex-shrink-0 flex items-center justify-center bg-green-100 rounded-full">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-slate-800 mt-1">{{ $activeSubscriptions }}</p>
            </div>
        </div>
    </div>

    <div class="mt-8 space-y-8" x-data="{ billing: 'monthly' }">
        <div class="flex items-center justify-center gap-4">
            ...
            <span :class="billing === 'annually' ? 'text-blue-600 font-semibold' : 'text-slate-500'">Tahunan <span class="text-xs font-medium text-green-600 bg-green-100 px-2 py-0.5 rounded-full">Hemat 20%</span></span>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @forelse ($packages as $package)
                @php 
                    $isPopular = ($package->id === ($mostPopularPackage->id ?? null));
                @endphp
                <div class="border rounded-2xl p-8 flex flex-col relative transition-all duration-300 {{ $isPopular ? 'border-blue-500 ring-2 ring-blue-500/50 shadow-2xl scale-105' : 'border-slate-200 bg-slate-50 shadow-lg hover:shadow-xl' }}">
                    @if($isPopular)
                        <p class="absolute -top-3 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg">Paling Populer</p>
                    @endif
                    ...
                    <div class="mt-6">
                        <span class="text-4xl font-bold tracking-tight text-slate-900" x-text="billing === 'monthly' ? 'Rp {{ number_format($package->price_monthly) }}' : 'Rp {{ number_format($package->price_annually) }}'"></span>
                        <span class="text-base font-medium text-slate-500" x-text="billing === 'monthly' ? '/bulan' : '/tahun'"></span>
                    </div>
                    ...
                    <ul role="list" class="mt-8 space-y-3 text-sm leading-6 text-slate-600 flex-grow">
                        <li class="flex gap-x-3">
                            ...
                            Durasi Aktif <strong>{{ $package->duration_in_days }} Hari</strong>
                        </li>
                        <li class="flex gap-x-3">
                            <svg class="h-6 w-5 flex-none text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" /></svg>
                            Maksimal <strong>{{ $package->max_letters ? number_format($package->max_letters) : 'Tak Terbatas' }}</strong> Surat
                        </li>
                         <li class="flex gap-x-3">
                            <svg class="h-6 w-5 flex-none text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" /></svg>
                            Maksimal <strong>{{ $package->max_patients ? number_format($package->max_patients) : 'Tak Terbatas' }}</strong> Pasien
                        </li>
                    </ul>
            ...
            @empty
                <div class="lg:col-span-3 text-center text-slate-500 py-16 bg-slate-50 rounded-xl border-2 border-dashed border-slate-200">
                    <svg class="w-16 h-16 mx-auto text-slate-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21 11.25v8.25a2.25 2.25 0 01-2.25 2.25H5.25a2.25 2.25 0 01-2.25-2.25v-8.25a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 11.25z" /></svg>
                    <p class="mt-4 font-semibold">Belum Ada Paket</p>
