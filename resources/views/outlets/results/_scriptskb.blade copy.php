...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fetchPatientSuggestions = debounce(function (query) {
        if (query.length >= 1) {
            fetch(`/patients/live-search?q=${encodeURIComponent(query)}`)
                .then(res => res.json())
                .then(data => {
    ...
    const fetchCompanySuggestions = debounce(function (query) {
        if (query.length >= 1) {
            fetch(`/companies/live-search?q=${encodeURIComponent(query)}`)
                .then(res => res.json())
                .then(data => {
    ...
    const fetchIcdSuggestions = debounce(function (query) {
        if (query.length >= 2) {
            fetch(`/icd10/live-search?q=${encodeURIComponent(query)}`)
                .then(res => res.json())
                .then(data => {
    ...
    @if(old('doctor_id') && old('doctor_name_display'))
        ...
        fetch(`/doctors/live-search?id={{ old('doctor_id') }}`)
            .then(res => res.json())
            .then(data => {
                if (data.length > 0) {
    ...
    const fetchDoctorSuggestions = debounce(function (query) {
        if (query.length >= 2) {
            fetch(`/doctors/live-search?q=${encodeURIComponent(query)}`)
                .then(res => res.json())
                .then(data => {
